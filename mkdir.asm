  ...
  17:	83 c3 04             	add    $0x4,%ebx
{
  ...
  25:	e8 f0 02 00 00       	call   31a <mkdir>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 38c:	f7 f1                	div    %ecx
 38e:	0f b6 92 88 07 00 00 	movzbl 0x788(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 518:	8b 55 d4             	mov    -0x2c(%ebp),%edx
        ...
        if(s == 0)
          s = "(null)";
 51b:	b8 81 07 00 00       	mov    $0x781,%eax
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
        ap++;
...
malloc(uint nbytes)
{
 ...
 679:	8b 55 08             	mov    0x8(%ebp),%edx
  if((prevp = freep) == 0){
 67c:	8b 0d 24 0a 00 00    	mov    0xa24,%ecx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
malloc(uint nbytes)
{
 ...
 6af:	76 4f                	jbe    700 <malloc+0x90>
    ...
    if(p == freep)
 6b1:	3b 05 24 0a 00 00    	cmp    0xa24,%eax
...
morecore(uint nu)
{
 ...
 6e7:	e8 f4 fe ff ff       	call   5e0 <free>
  return freep;
 6ec:	8b 0d 24 0a 00 00    	mov    0xa24,%ecx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 70c:	89 58 04             	mov    %ebx,0x4(%eax)
      ...
      freep = prevp;
 70f:	89 0d 24 0a 00 00    	mov    %ecx,0xa24
      return (void*)(p + 1);
 ...
 730:	b8 28 0a 00 00       	mov    $0xa28,%eax
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 735:	c7 05 28 0a 00 00 28 	movl   $0xa28,0xa28
 73c:	0a 00 00 
    base.s.size = 0;

...
main(void)
{
  ...
  for(;;){
    ...
    if(pid < 0){
      printf(1, "init: fork failed\n");
  8c:	c7 44 24 04 f9 07 00 	movl   $0x7f9,0x4(%esp)
  93:	00 
  94:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  9b:	e8 e0 03 00 00       	call   480 <printf>
      exit();
  a0:	e8 9d 02 00 00       	call   342 <exit>
    }
    if(pid == 0){
      exec("sh", argv);
  a5:	c7 44 24 04 c8 0a 00 	movl   $0xac8,0x4(%esp)
  ac:	00 
  ad:	c7 04 24 0c 08 00 00 	movl   $0x80c,(%esp)
  b4:	e8 c1 02 00 00       	call   37a <exec>
      printf(1, "init: exec sh failed\n");
  b9:	c7 44 24 04 0f 08 00 	movl   $0x80f,0x4(%esp)
  c0:	00 
  c1:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  c8:	e8 b3 03 00 00       	call   480 <printf>
      exit();
  cd:	e8 70 02 00 00       	call   342 <exit>
main(void)
{
  int pid, wpid;
...
strcmp(const char *p, const char *q)
{
 ...
 153:	75 16                	jne    16b <strcmp+0x2b>
 155:	eb 21                	jmp    178 <strcmp+0x38>
 157:	90                   	nop
    p++, q++;
...
{
  while(*p && *p == *q)
 15b:	0f b6 59 01          	movzbl 0x1(%ecx),%ebx
 15f:	0f b6 02             	movzbl (%edx),%eax
    p++, q++;
 162:	8d 71 01             	lea    0x1(%ecx),%esi
}
...
gets(char *buf, int max)
{
 ...
 224:	7d 42                	jge    268 <gets+0x58>
    cc = read(0, &c, 1);
 226:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
 22d:	00 
 22e:	89 7c 24 04          	mov    %edi,0x4(%esp)
 232:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
 239:	e8 1c 01 00 00       	call   35a <read>
    if(cc < 1)
 23e:	85 c0                	test   %eax,%eax
 240:	7e 26                	jle    268 <gets+0x58>
      break;
    buf[i++] = c;
 242:	0f b6 45 e7          	movzbl -0x19(%ebp),%eax
 246:	89 f3                	mov    %esi,%ebx
 ...
 24b:	3c 0d                	cmp    $0xd,%al
  ...
  for(i=0; i+1 < max; ){
    ...
    if(cc < 1)
      break;
    buf[i++] = c;
 24d:	88 44 32 ff          	mov    %al,-0x1(%edx,%esi,1)
    if(c == '\n' || c == '\r')
 251:	74 04                	je     257 <gets+0x47>
 253:	3c 0a                	cmp    $0xa,%al
 255:	75 c7                	jne    21e <gets+0xe>
      break;
  }
  buf[i] = '\0';
 257:	8b 45 08             	mov    0x8(%ebp),%eax
 25a:	c6 04 30 00          	movb   $0x0,(%eax,%esi,1)
  return buf;
}
...
stat(char *n, struct stat *st)
{
 ...
 289:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
 290:	00 
  return buf;
}
...
stat(char *n, struct stat *st)
{
 291:	89 5d f8             	mov    %ebx,-0x8(%ebp)
 294:	89 75 fc             	mov    %esi,-0x4(%ebp)
  int fd;
  int r;

  fd = open(n, O_RDONLY);
 297:	89 04 24             	mov    %eax,(%esp)
 29a:	e8 e3 00 00 00       	call   382 <open>
  if(fd < 0)
...
stat(char *n, struct stat *st)
{
 ...
 2a1:	89 c3                	mov    %eax,%ebx
  if(fd < 0)
 2a3:	78 2b                	js     2d0 <stat+0x50>
    return -1;
  r = fstat(fd, st);
 2a5:	8b 45 0c             	mov    0xc(%ebp),%eax
 2a8:	89 1c 24             	mov    %ebx,(%esp)
 2ab:	89 44 24 04          	mov    %eax,0x4(%esp)
 2af:	e8 e6 00 00 00       	call   39a <fstat>
  close(fd);
 ...
 2b7:	89 c6                	mov    %eax,%esi
  close(fd);
 2b9:	e8 ac 00 00 00       	call   36a <close>
  return r;
}
 2be:	89 f0                	mov    %esi,%eax
 2c0:	8b 5d f8             	mov    -0x8(%ebp),%ebx
 2c3:	8b 75 fc             	mov    -0x4(%ebp),%esi
 2c6:	89 ec                	mov    %ebp,%esp
 2c8:	5d                   	pop    %ebp
 2c9:	c3                   	ret    
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 40b:	89 d0                	mov    %edx,%eax
  ...
  if(sgn && xx < 0){
    neg = 1;
 40d:	bb 01 00 00 00       	mov    $0x1,%ebx
    x = -xx;
 412:	f7 d8                	neg    %eax
  } else {
    x = xx;
 ...
 416:	eb 02                	jmp    41a <printint+0x2a>
  do{
    buf[i++] = digits[x % base];
 418:	89 d6                	mov    %edx,%esi
 41a:	31 d2                	xor    %edx,%edx
 41c:	f7 f1                	div    %ecx
 41e:	0f b6 92 35 08 00 00 	movzbl 0x835(%edx),%edx
  }while((x /= base) != 0);
 425:	85 c0                	test   %eax,%eax
  ...
  do{
    buf[i++] = digits[x % base];
 427:	88 54 35 d8          	mov    %dl,-0x28(%ebp,%esi,1)
 42b:	8d 56 01             	lea    0x1(%esi),%edx
  }while((x /= base) != 0);
 42e:	75 e8                	jne    418 <printint+0x28>
  if(neg)
 430:	85 db                	test   %ebx,%ebx
 432:	74 08                	je     43c <printint+0x4c>
    buf[i++] = '-';
 434:	c6 44 15 d8 2d       	movb   $0x2d,-0x28(%ebp,%edx,1)
 439:	8d 56 02             	lea    0x2(%esi),%edx

  while(--i >= 0)
 43c:	8d 5a ff             	lea    -0x1(%edx),%ebx
 43f:	83 fb ff             	cmp    $0xffffffff,%ebx
 442:	8d 75 d7             	lea    -0x29(%ebp),%esi
 445:	74 25                	je     46c <printint+0x7c>
 447:	90                   	nop
 448:	0f b6 44 1d d8       	movzbl -0x28(%ebp,%ebx,1),%eax
 44d:	83 eb 01             	sub    $0x1,%ebx
#include "user.h"
...
putc(int fd, char c)
{
 ...
 45f:	88 45 d7             	mov    %al,-0x29(%ebp)
 462:	e8 fb fe ff ff       	call   362 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
  ...
  while(--i >= 0)
 467:	83 fb ff             	cmp    $0xffffffff,%ebx
 46a:	75 dc                	jne    448 <printint+0x58>
    putc(fd, buf[i]);
}
 ...
 47c:	eb 96                	jmp    414 <printint+0x24>
...
{
  write(fd, &c, 1);
 4c6:	e8 97 fe ff ff       	call   362 <write>
 4cb:	83 c3 01             	add    $0x1,%ebx
  ...
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
 4ce:	0f be 43 ff          	movsbl -0x1(%ebx),%eax
...
{
  write(fd, &c, 1);
 553:	e8 0a fe ff ff       	call   362 <write>
  int c, i, state;
  uint *ap;
  ...
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
 558:	0f be 43 ff          	movsbl -0x1(%ebx),%eax
 ...
 595:	e8 56 fe ff ff       	call   3f0 <printint>
        ap++;
 ...
 5a8:	8b 55 d4             	mov    -0x2c(%ebp),%edx
        ...
        if(s == 0)
          s = "(null)";
 5ab:	b8 2e 08 00 00       	mov    $0x82e,%eax
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
        ap++;
        ...
        s = (char*)*ap;
 5b0:	8b 3a                	mov    (%edx),%edi
        ap++;
 5b2:	83 c2 04             	add    $0x4,%edx
 5b5:	89 55 d4             	mov    %edx,-0x2c(%ebp)
        if(s == 0)
          s = "(null)";
 5b8:	85 ff                	test   %edi,%edi
 5ba:	0f 44 f8             	cmove  %eax,%edi
        while(*s != 0){
 5bd:	0f b6 07             	movzbl (%edi),%eax
 5c0:	84 c0                	test   %al,%al
 5c2:	74 28                	je     5ec <printf+0x16c>
...
putc(int fd, char c)
{
 ...
 636:	e8 b5 fd ff ff       	call   3f0 <printint>
        ap++;
...
malloc(uint nbytes)
{
 ...
 709:	8b 55 08             	mov    0x8(%ebp),%edx
  if((prevp = freep) == 0){
 70c:	8b 0d d0 0a 00 00    	mov    0xad0,%ecx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 712:	83 c2 07             	add    $0x7,%edx
 715:	c1 ea 03             	shr    $0x3,%edx
  if((prevp = freep) == 0){
...
malloc(uint nbytes)
{
  ...
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 71a:	8d 5a 01             	lea    0x1(%edx),%ebx
  if((prevp = freep) == 0){
 71d:	0f 84 93 00 00 00    	je     7b6 <malloc+0xb6>
 723:	8b 01                	mov    (%ecx),%eax
 725:	8b 78 04             	mov    0x4(%eax),%edi
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    if(p->s.size >= nunits){
 728:	39 fb                	cmp    %edi,%ebx
 ...
 735:	8d 76 00             	lea    0x0(%esi),%esi
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 738:	8b 01                	mov    (%ecx),%eax
    if(p->s.size >= nunits){
 73a:	8b 78 04             	mov    0x4(%eax),%edi
 73d:	39 fb                	cmp    %edi,%ebx
 73f:	76 4f                	jbe    790 <malloc+0x90>
    ...
    if(p == freep)
 741:	3b 05 d0 0a 00 00    	cmp    0xad0,%eax
...
morecore(uint nu)
{
 ...
 756:	0f 43 c6             	cmovae %esi,%eax
 759:	bf 00 10 00 00       	mov    $0x1000,%edi
    nu = 4096;
...
morecore(uint nu)
{
  ...
  if(nu < 4096)
 761:	0f 43 fb             	cmovae %ebx,%edi
    nu = 4096;
  p = sbrk(nu * sizeof(Header));
 764:	e8 61 fc ff ff       	call   3ca <sbrk>
  if(p == (char*)-1)
 769:	83 f8 ff             	cmp    $0xffffffff,%eax
 76c:	74 18                	je     786 <malloc+0x86>
    return 0;
  hp = (Header*)p;
  hp->s.size = nu;
 ...
 777:	e8 f4 fe ff ff       	call   670 <free>
  return freep;
 77c:	8b 0d d0 0a 00 00    	mov    0xad0,%ecx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 784:	75 b2                	jne    738 <malloc+0x38>
        return 0;
 786:	31 c0                	xor    %eax,%eax
 788:	eb 1e                	jmp    7a8 <malloc+0xa8>
 78a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    if(p->s.size >= nunits){
      if(p->s.size == nunits)
 790:	39 fb                	cmp    %edi,%ebx
 792:	74 1c                	je     7b0 <malloc+0xb0>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
 794:	29 df                	sub    %ebx,%edi
 796:	89 78 04             	mov    %edi,0x4(%eax)
        p += p->s.size;
 799:	8d 04 f8             	lea    (%eax,%edi,8),%eax
        p->s.size = nunits;
 79c:	89 58 04             	mov    %ebx,0x4(%eax)
      }
      freep = prevp;
 79f:	89 0d d0 0a 00 00    	mov    %ecx,0xad0
      return (void*)(p + 1);
 ...
 7b4:	eb e9                	jmp    79f <malloc+0x9f>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7b6:	c7 05 d0 0a 00 00 d4 	movl   $0xad4,0xad0
 7bd:	0a 00 00 
    base.s.size = 0;
 7c0:	b8 d4 0a 00 00       	mov    $0xad4,%eax
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7c5:	c7 05 d4 0a 00 00 d4 	movl   $0xad4,0xad4
 7cc:	0a 00 00 
    base.s.size = 0;
